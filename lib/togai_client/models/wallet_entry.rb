  ...
  class WalletEntry
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TogaiClient::WalletEntry` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TogaiClient::WalletEntry`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      transaction_type_validator = EnumAttributeValidator.new('String', ["CREDITED", "DEBITED"])
      return false unless transaction_type_validator.valid?(@transaction_type)
      return false if @status.nil?
      status_validator = EnumAttributeValidator.new('String', ["COMPLETED", "ON_HOLD"])
    ...
    def transaction_type=(transaction_type)
      validator = EnumAttributeValidator.new('String', ["CREDITED", "DEBITED"])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["COMPLETED", "ON_HOLD"])
    ...
    def hash
      [id, description, wallet_id, transaction_type, status, entity_id, amount, created_at, closing_balance].hash
    end
