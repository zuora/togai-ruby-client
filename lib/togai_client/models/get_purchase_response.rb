  ...
  class GetPurchaseResponse
    ...
    def self.openapi_types
      {
        ...
        :'type' => :'PurchaseType',
        :'comment' => :'String',
        :'purchase_plan' => :'PricePlanDetails',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TogaiClient::GetPurchaseResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TogaiClient::GetPurchaseResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'purchase_plan')
        self.purchase_plan = attributes[:'purchase_plan']
      end
