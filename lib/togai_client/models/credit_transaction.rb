  ...
  class CreditTransaction
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TogaiClient::CreditTransaction` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TogaiClient::CreditTransaction`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      transaction_type_validator = EnumAttributeValidator.new('String', ["CREDITED", "DEBITED", "EXPIRED", "VOIDED"])
    ...
    def transaction_type=(transaction_type)
      validator = EnumAttributeValidator.new('String', ["CREDITED", "DEBITED", "EXPIRED", "VOIDED"])
    ...
    def hash
      [id, credit_id, transaction_type, invoice_id, entity_id, amount, created_at].hash
    end
