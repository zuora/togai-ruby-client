...
module TogaiClient
  class UpdatePricingScheduleRequest
    ...
    attr_accessor :pricing_rules_override
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TogaiClient::UpdatePricingScheduleRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TogaiClient::UpdatePricingScheduleRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def mode=(mode)
      validator = EnumAttributeValidator.new('String', ["ASSOCIATE", "DISASSOCIATE"])
    ...
    def hash
      [mode, price_plan_id, effective_from, effective_until, price_plan_details_override, pricing_rules_override, retain_start_offsets].hash
    end
