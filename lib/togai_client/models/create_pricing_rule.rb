  ...
  class CreatePricingRule
    ...
    def self.openapi_types
      {
        :'name' => :'String',
        :'order' => :'Integer',
        :'invoice_timing' => :'PricingRuleTiming',
        :'condition' => :'String',
        :'computation' => :'String',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TogaiClient::CreatePricingRule` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TogaiClient::CreatePricingRule`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [name, order, invoice_timing, condition, computation, action].hash
    end
