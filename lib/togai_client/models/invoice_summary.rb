  ...
  class InvoiceSummary
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TogaiClient::InvoiceSummary` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TogaiClient::InvoiceSummary`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, customer_id, owner_id, price_plan_id, status, finalizing_status, invoice_class, invoice_type, start_date, end_date, end_date_inclusive, invoice_date, due_date, generated_at, updated_at, sequence_id, pdf_url, total_amount, paid_amount, invoice_details, net_term_days].hash
    end
