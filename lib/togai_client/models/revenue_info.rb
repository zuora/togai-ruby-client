  ...
  class RevenueInfo
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TogaiClient::RevenueInfo` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TogaiClient::RevenueInfo`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [usage_rate_card, fixed_fee_rate_card, license_rate_card, billing_entitlement_rate_card, credit_grant_rate_card, entitlement_overage_rate_card, usages, fixed_fee_revenue_summary, license_revenue_summary, billing_entitlement_revenue_summary, credit_grant_revenue_summary, entitlement_overage_revenue_summary, slab_revenue_summaries].hash
    end
