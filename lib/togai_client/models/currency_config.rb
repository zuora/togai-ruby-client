  ...
  class CurrencyConfig
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TogaiClient::CurrencyConfig` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TogaiClient::CurrencyConfig`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@currency.nil? && @currency.to_s.length < 1
    ...
    def valid?
      ...
      mode_validator = EnumAttributeValidator.new('String', ["CUSTOM", "ACCOUNT_INVOICE"])
      return false unless mode_validator.valid?(@mode)
      return false if !@currency.nil? && @currency.to_s.length < 1
      true
    end
    ...
    def mode=(mode)
      validator = EnumAttributeValidator.new('String', ["CUSTOM", "ACCOUNT_INVOICE"])
