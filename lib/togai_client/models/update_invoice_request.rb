  ...
  class UpdateInvoiceRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TogaiClient::UpdateInvoiceRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TogaiClient::UpdateInvoiceRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      status_validator = EnumAttributeValidator.new('String', ["DUE", "PAID", "VOID", "UN_COLLECTIBLE", "REFUND_INITIATED"])
      return false unless status_validator.valid?(@status)
      return false if !@line_items.nil? && @line_items.length > 50
      return false if !@line_items.nil? && @line_items.length < 1
      true
    end
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["DUE", "PAID", "VOID", "UN_COLLECTIBLE", "REFUND_INITIATED"])
    ...
    def hash
      [start_date, end_date, status, line_items, auto_advance].hash
    end
