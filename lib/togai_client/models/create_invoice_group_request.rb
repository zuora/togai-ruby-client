...
module TogaiClient
  ...
  class CreateInvoiceGroupRequest
    attr_accessor :name
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TogaiClient::CreateInvoiceGroupRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TogaiClient::CreateInvoiceGroupRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @account_ids.length > 100
        invalid_properties.push('invalid value for "account_ids", number of items must be less than or equal to 100.')
      end
    ...
    def valid?
      ...
      return false if @account_ids.length > 100
    ...
    def account_ids=(account_ids)
      ...
      if account_ids.length > 100
        fail ArgumentError, 'invalid value for "account_ids", number of items must be less than or equal to 100.'
      end
    ...
    def hash
      [name, email, daily_invoice_consolidation, account_ids, net_term_days, address].hash
    end
